...
void Alarm::handler(const IC::Interrupt_Id & i)
{
    ...
    if(!_request.empty()) {
        ...
        while (int(_request.head()->rank()) <= 0 && index < 50) { //rank can be negative whenever multiple handlers get created for the same time tick
            Queue::Element * e = _request.remove();
            alarm[index] = e->object();
            if(alarm[index]->_times != INFINITE)
                alarm[index]->_times--;
            if(alarm[index]->_times) {
                e->rank(alarm[index]->_ticks);
                _request.insert(e);
            }
            index++;
            if(_request.empty())
    ...
    while (index > 0) {
        index--;
        db<Alarm>(TRC) << "Alarm::handler(this=" << alarm[index] << ",e=" << _elapsed << ",h=" << reinterpret_cast<void*>(alarm[index]->handler) << ")" << endl;
        (*alarm[index]->_handler)();
    }
    // */
    // /*
    Alarm * alarm = 0;
